...
def extract_items_via_llm(user_query, context, max_items=3):
    ...
    out = together_chat([{"role":"system","content":JSON_SYSTEM},{"role":"user","content":prompt}],temperature=0.0, max_tokens=700).strip()
...
def build_and_render_offer_pdf(offer_id, customer_name, customer_no, items, vat_rate=0.0, default_currency="USD", filename="offer.pdf"):
    ...
    draw_text(page, col_dim,  y, "Dimensions", fs_small)   # <<< CHANGED: header is visible
...
def inject_dimensions_into_items(items: list, note_dims: dict) -> list:
    ...
    for it in items:
        ...
        if key and note_dims.get(key):
            it["dimensions"] = note_dims[key]    # <<< CHANGED: always overwrite to ensure visibility
    return items
...
if __name__ == "__main__":
    ...
    # Parse & inject (overwrite) dimensions from the user's note  <<< CHANGED
