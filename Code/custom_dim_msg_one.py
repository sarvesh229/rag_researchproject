...
- If no exact items, ask for similar; on 'no', print a friendly message and exit
- NEW: When proceeding with similar/custom items, generate EXACTLY ONE closest item
"""
...
def match_product(user_query, faiss_store, bm25_retriever, all_chunks, full_docs):
    ...
    # Ensure only the single best doc is used downstream
    top_docs = top_docs[:1]
...
# ----------------------------- #
# Single-item selection helpers
# ----------------------------- #
WANT_TYPES = ["bed","sofa","couch","chair","table","desk","wardrobe","cabinet",
              "shelf","bookshelf","dresser","nightstand","stool","bench","futon","recliner"]
WANT_COLORS = ["black","white","brown","oak","walnut","grey","gray","beige","natural","espresso","red","blue","copper"]
WANT_MATERIALS = ["wood","wooden","engineered wood","bamboo","rattan","teak","metal","steel",
                  "iron","plywood","leather","fabric","plastic","glass","oak","pine","mdf",
                  "particleboard","foam"]

def desire_from_text(text: str) -> dict:
    t = (text or "").lower()
    want_type = next((w for w in WANT_TYPES if w in t), None)
    want_color = next((w for w in WANT_COLORS if w in t), None)
    want_mat   = next((w for w in WANT_MATERIALS if w in t), None)
    return {"type": want_type, "color": want_color, "material": want_mat}

def score_item_against_desire(item: Dict[str, Any], want: dict) -> int:
    """Simple, robust scoring: +3 if type appears in name/desc; +2 for material; +1 for color."""
    text = " ".join([
        str(item.get("name","")),
    ...
    s = 0
    if want.get("type") and want["type"] in text: s += 3
    if want.get("material") and want["material"] in text: s += 2
    if want.get("color") and want["color"] in text: s += 1
    return s

def pick_best_single_item(user_query: str, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Return a list with exactly the top 1 item."""
    if not items: return []
    want = desire_from_text(user_query)
    ranked = sorted(items, key=lambda it: score_item_against_desire(it, want), reverse=True)
    top = ranked[0]
    top["pos"] = 1
    return [top]

# ----------------------------- #
# Rule-based extraction backstop
# ----------------------------- #
...
def extract_items(user_query, context, max_items=3):
    ...
    # Final guard: ensure at most 1 when asked
    if max_items == 1 and len(items) > 1:
        items = pick_best_single_item(user_query, items)
...
if __name__ == "__main__":
    ...
    if not multi_note:
        ...
        # If no exact/usable matches, ask whether to proceed with similar/custom items
        if match_type == "none":
            ...
            # Use only the top-1 matched doc and extract only 1 item
            matched_docs = matched_docs[:1]
            context = matched_docs[0].page_content[:1500]
            items = extract_items(user_query, context, max_items=1)
            # Keep just the single best item (safety)
            items = pick_best_single_item(user_query, items)
    ...
    if multi_note:
        ...
        # pick the closest single item to the user's intent (type/material/color)
        pseudo_items = [
            {
                "name": b.get("name") or b.get("category") or "",
                "description": b.get("color") or "",
                "material": " / ".join(sorted({m.get("name","") for m in b.get("materials",[]) if m.get("name")})),
                "brand": None
            }
            for b in bom_items
        ]
        one_bom = pick_best_single_item(user_query, pseudo_items)

        # Map back to the original bom specs (so pricing uses the selected item only)
        if one_bom:
            chosen_name = one_bom[0]["name"].lower()
            def _sim(b):
                base = (b.get("name") or b.get("category") or "").lower()
                return difflib.SequenceMatcher(None, base, chosen_name).ratio()
            best_spec = sorted(bom_items, key=_sim, reverse=True)[0:1]
        else:
            best_spec = bom_items[0:1]  # fallback to first

        items = build_priced_items_from_bom(best_spec, price_map, material_to_units)

    if not items:
        print(DECLINE_MSG)
        raise SystemExit(0)

    # Parse & inject (overwrite) dimensions from the user's note
