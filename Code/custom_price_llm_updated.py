# -- coding: utf-8 --
"""
Offer generator (RAG + Excel-priced BOM fallback, per-item catalog→excel→decline pipeline)

What this script does:
1) Parse ALL items from the user's free-text note (name/category/qty/color/dimensions/materials).
2) For each item:
   a) Try to retrieve a matching catalog item from your embedded PDFs and extract a priced line.
   b) If catalog match/pricing fails, compute price from your Excel using the LLM (with density/thickness conversions).
   c) If both fail, stop and print the decline message.
3) Build a PDF with Description + Dimensions and a "Pricing: $X × Qty N = $Y" line for each item.

Hard rules:
- NEVER output $0.00: if LLM returns 0/invalid, deterministic fallback pricing is applied.
- Dimensions (W/D/H or Ø; H range) are guaranteed to print when present in the note or BOM.
"""
...
# ----------------------------- #
FURNITURE_TYPES = ["bed","sofa","couch","chair","table","desk","wardrobe","cabinet","shelf","bookshelf","dresser","nightstand","stool","bench","futon","recliner","mirror","lamp"]
COLORS = ["black","white","brown","oak","walnut","grey","gray","beige","natural","espresso","red","blue","copper","dark oak","dark walnut"]
MATERIALS = ["wood","wooden","engineered wood","bamboo","rattan","teak","metal","steel","iron","plywood","leather","fabric","plastic","glass","oak","pine","mdf","particleboard","foam","glass fiber","fiberglass"]
...
def looks_like_main_furniture(text: str, intent: dict) -> bool:
    ...
    good = {"chair","sofa","couch","table","desk","bench","stool","bed","wardrobe","futon","recliner","shelf","nightstand","mirror","lamp"}
...
def extract_items_via_llm(user_query, context, max_items=1):
    ...
    try:
        ...
        if DEBUG: print("LLM JSON parse failed for catalog:\n", out[:300], "...")
        return {"items":[]}

def looks_like_title_line(s:str)->bool:
    s=s.strip()
    if not s or re.match(r'^\s*(€|\$|usd|eur)\b',s.lower()): return False
    return 8<=len(s)<=200 and len(re.findall(r'[A-Za-z]',s))>=6

def _title_score(line: str, intent_type: Optional[str]) -> int:
    s=line.strip()
    if len(s)<6 or len(s)>120 or re.search(r"^\s*(€|\$|\d)", s): return -5
    score=1
...
def extract_items_rule_based(context:str,intent:dict,max_items=1):
    ...
    for i,line in enumerate(lines):
        ...
        items.append({"pos":len(items)+1,"qty":1,"unit":"pc","name":title or "Item",
                      "material":None,"dimensions":None,"brand":None,
                      "description":"","price":amount,"currency":cur})
        if len(items)>=max_items: break
    return items

def catalog_best_item_for_spec(spec: Dict[str, Any],
                               faiss_store, bm25_retriever, all_chunks, full_docs) -> Optional[Dict[str, Any]]:
    """
    ...
    # take the best chunk and try LLM extraction (with rule fallback)
    d = cands[0]
    ...
    # normalize to our line format
    it = items[0]
    qty = int(spec.get("qty") or 1)
    # if catalog has price use it; else fail (so we fall back to Excel path)
    if it.get("price") in (None, "", 0, 0.0):
        return None

    # dimensions (if catalog gave any) OR compute from spec
    dims_txt = None
    if it.get("dimensions"):
        ...
        if "diameter" in dims:
            ...
            if "h_min" in dims and "h_max" in dims: p.append(f'{dims["h_min"]}-{dims["h_max"]}H cm')
            elif "h" in dims: p.append(f'{dims["h"]}H cm')
            dims_txt="; ".join(p)
        else:
            if "w" in dims: parts.append(f'{dims["w"]}W')
            if "d" in dims: parts.append(f'{dims["d"]}D')
            if "h" in dims: parts.append(f'{dims["h"]}H')
            if parts: dims_txt=" x ".join(parts) + " (cm)"
...
# ----------------------------- #
REQUIRED_COLS={"material_name","unit","unit_price_usd"}  # others optional (color, density_kg_per_m3, thickness_mm)
...
def load_material_costs_xlsx(path=COST_BOOK_PATH,sheet:Optional[str]=None):
    ...
    price_map={f"{row['material_name_norm']}|{row['unit_norm']}": float(row["unit_price_usd"])
               for _,row in df.iterrows() if pd.notna(row["unit_price_usd"])}

    material_to_units:Dict[str,Dict[str,float]]={}
    for _,row in df.iterrows():
        m=row["material_name_norm"]; u=row["unit_norm"]; p=row["unit_price_usd"]
        if pd.notna(p):
...
# ----------------------------- #
# LLM-based pricing with full Excel rows
# ----------------------------- #
...
def llm_compute_prices_for_bom(
    ...
    system = (
        "Return ONLY a JSON object with an 'items' array. No prose. "
        "Use only the provided price table rows for prices. "
        "If units differ, convert using density_kg_per_m3 or thickness_mm when available. "
        "Round monetary outputs to 2 decimals."
    )

    rules = {
        "selection": [
            "Match material_name (case-insensitive). If BOM has color and table has color, prefer same color.",
            "If multiple rows match, choose the most specific."
        ],
        "conversions": [
            "If BOM unit == 'm3' and table unit == 'kg' and density_kg_per_m3 is available: unit_price_m3 = unit_price_usd * density_kg_per_m3.",
            "If BOM unit == 'kg' and table unit == 'm3' and density_kg_per_m3 is available: unit_price_kg = unit_price_usd / density_kg_per_m3.",
            "If BOM unit == 'm2' and table unit == 'm3' and thickness_mm is available: m3 = m2 * (thickness_mm / 1000).",
            "If BOM unit == 'm2' and table unit == 'm2': use table price as-is.",
            "If conversion info is missing, treat that material's contribution as 0."
        ],
        "pricing": [
            "raw_unit_cost = sum(material.quantity * converted_unit_price for each material).",
        ...
        "output_schema": {"items":[
            {"name":"", "category":"", "qty":1, "unit_price_est":0.0, "price":0.0}
        ]}
    }
    ...
    def _ask(minimal=False):
        msgs = [
            {"role":"system","content": "Return ONLY JSON. No markdown, no comments." if minimal else system},
            {"role":"user","content": json.dumps(payload, ensure_ascii=False)}
        ]
        out = together_chat(msgs, temperature=0.0, max_tokens=1800).strip()
        return extract_json_object(out)

    obj = _ask(minimal=False) or _ask(minimal=True)
    if not obj or not isinstance(obj.get("items"), list):
        if DEBUG: print("⚠ LLM pricing parse failed.")
        return None
    return obj["items"]
...
def build_priced_items_from_bom_llm(
    ...
    for spec in items_bom:
        ...
        # per-line fallback if LLM gave 0/negative/invalid or missing
...
RULES:
- Use ONLY materials from the allowed list. Units: wood/mdf/pine/foam in m³; metal/steel in kg; leather/fabric/glass/marble in m²; plastic/fiberglass in m³.
- Each item: name, category, qty, color, dimensions_cm (known keys among w,d,h,diameter,h_min,h_max), materials [{{name,unit,quantity}}].
- Respect counts in the note. Do not invent extra items. Return JSON only.
...
{{
  "items": [
    ...
    {{
      ...
      "materials": [{{"name":"glass fiber","unit":"m³","quantity":0.06}}, {{"name":"foam","unit":"m³","quantity":0.08}}, {{"name":"fabric","unit":"m²","quantity":6.0}}]
    }},
    {{
...
# ----------------------------- #
# NOTE → DIMENSIONS HELPERS (improved W/D/H)
# ----------------------------- #
DIM_MM = r'(\d{2,4})\s*mm'
...
def extract_dimensions_from_note(note: str) -> dict:
    ...
    # Bed: width/height/depth in mm anywhere after 'bed'
    m = re.search(r'bed[^.]*?width[^0-9]*'+DIM_MM+r'[^.]*?height[^0-9]*'+DIM_MM+r'[^.]*?depth[^0-9]*'+DIM_MM, s)
    if m:
        w = mm_to_cm(m.group(1)); h = mm_to_cm(m.group(2)); d = mm_to_cm(m.group(3))
        parts=[]
        if w is not None: parts.append(f"{w}W")
        if d is not None: parts.append(f"{d}D")
        if h is not None: parts.append(f"{h}H")
        if parts: dims['bed'] = " x ".join(parts) + " (cm)"

    # Sofa: wide/width, deep/depth, high/height in mm
    m = re.search(r'sofa[^.]*?(?:wide|width)[^0-9]*'+DIM_MM+r'[^.]*?(?:deep|depth)[^0-9]*'+DIM_MM+r'[^.]*?(?:high|height)[^0-9]*'+DIM_MM, s)
    if m:
        w = mm_to_cm(m.group(1)); d = mm_to_cm(m.group(2)); h = mm_to_cm(m.group(3))
        parts=[]
        if w is not None: parts.append(f"{w}W")
        if d is not None: parts.append(f"{d}D")
        if h is not None: parts.append(f"{h}H")
    ...
    # Nightstand: width + height in mm
    m = re.search(r'nightstand[^.]*?(?:wide|width)[^0-9]*'+DIM_MM+r'[^.]*?(?:high|height)[^0-9]*'+DIM_MM, s)
    if m:
        w = mm_to_cm(m.group(1)); h = mm_to_cm(m.group(2))
        parts=[]
        if w is not None: parts.append(f"{w}W")
        if h is not None: parts.append(f"{h}H")
        if parts: dims['nightstand'] = " x ".join(parts) + " (cm)"

    # Desk (height only)
    m = re.search(r'desk[^.]*?'+DIM_MM+r'[^.]*?(?:tall|high|height)', s)
    if m:
        h = mm_to_cm(m.group(1))
        if h is not None: dims['desk'] = f"{h}H (cm)"

    # Round table (diameter + height range in cm)
    ...
    if m_d:
        ...
        if d_cm is not None: parts.append(f"{d_cm}Ø")
    if m_h:
...
if __name__ == "__main__":
    ...
    # Load data
    price_map, material_to_units, allowed_materials, df_cost = load_material_costs_xlsx(COST_BOOK_PATH, EXCEL_SHEET)
    ...
    # 2) For each item: catalog → excel → decline
    final_items: List[Dict[str, Any]] = []
    unavailable = False

    for spec in bom_items:
        # Try catalog
        cat_item = catalog_best_item_for_spec(spec, faiss_store, bm25_retriever, chunked_docs, full_docs)

        if cat_item:
            final_items.append(cat_item)
            continue

        # Else price from Excel via LLM (with deterministic fallback guard)
        priced = build_priced_items_from_bom([spec], price_map, material_to_units, df=df_cost)
    ...
    # 3) Inject dimensions parsed directly from note (overwrite where relevant)
