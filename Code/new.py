# -*- coding: utf-8 -*-
"""
Offer generator (agentic-ready RAG pipeline)
- Retrieval: FAISS (dense) + BM25 (lexical) + literal-phrase booster + (optional) cross-encoder rerank
- Extraction: LLM JSON with robust rule-based fallback (stitched multi-line titles)
- Attributes: Material / Dimensions / Brand extracted from a window above & below price (and inline patterns)
- Dedupe: collapse near-identical items
- Output: A4 PDF with right-aligned price column
- Output folder: generated_offers/
- Precision guard: Accessory / non-furniture filtering (prevents empty/irrelevant offers)
...
def load_offer_documents(folder_path):
    ...
    for file in os.listdir(folder_path):
        ...
        # Whole-file doc for literal phrase boost
        ...
        # Chunked docs with filename prefix (to keep identity)
        prepared = []
        for d in raw_docs:
...
EXCLUDE_KEYWORDS = [
    ...
    "kit","spare","part","accessory","accessories","fastener","screw","bracket","joint"
]

def looks_like_main_furniture(text: str, intent: dict) -> bool:
    """
    True if the text likely refers to an intended furniture item (not accessory).
    """
    low = text.lower()
    t = intent.get("type")
    ...
    if any(bad in low for bad in EXCLUDE_KEYWORDS):
        return False
    good_terms = {"chair","sofa","couch","table","desk","bench","stool","bed","wardrobe"}
...
def match_product(user_query, faiss_store, bm25_retriever, all_chunks, full_docs):
    ...
    # 1) Phrase booster on whole files
    ...
    # 3.5) NEW: filter out accessory / non-furniture docs — NO fallback
    ...
    merged = pre_filtered  # <- important: no "or merged" fallback

    # If nothing valid remains, exit early
    if not merged:
        return "none", []
...
# ----------------------------- #
# Rule-based fallback with stitched titles + robust attrs
# ----------------------------- #
...
if __name__ == "__main__":
    ...
    # If nothing valid survived filtering, stop BEFORE any prompt
    if match_type == "none" or not matched_docs:
        print("❌ No suitable furniture items matched your query (accessories filtered or no hit).")
        raise SystemExit(0)

    # We’ll only preview/confirm using valid docs (already filtered in match_product)
    ...
    # Build context for extraction from the already-filtered docs
    ...
    if not items:
        print("❌ Could not extract any valid line items. Try a more specific query.")
        raise SystemExit(0)
