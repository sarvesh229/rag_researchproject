...
UPDATES:
- Show "Unit est.: $X × Qty N = $Y" per line.
- Never print "?" in dimensions; omit missing parts.
...
def extract_items_via_llm(user_query, context, max_items=3):
    ...
    out = together_chat([{"role":"system","content":JSON_SYSTEM},{"role":"user","content":prompt}],0.0,700).strip()
...
def price_from_bom(spec: Dict[str,Any], price_map: Dict[str,float], material_to_units: Dict[str,Dict[str,float]]) -> float:
    ...
    for m in spec.get("materials",[]):
        ...
        if price is None:
            if DEBUG: print(f"⚠️ Missing price for material: {name} ({unit})")
            continue
...
def build_and_render_offer_pdf(offer_id, customer_name, customer_no, items, vat_rate=0.0, default_currency="USD", filename="offer.pdf"):
    ...
    col_qty = col_pos + 40
    col_desc = col_qty + 44
    col_dim  = col_desc + 240         # width for description ≈ 240pt
    col_price = W - RM
    ...
    for idx, it in enumerate(items, 1):
        ...
        dims_txt = it.get("dimensions") or ""    # may be empty; we still have the column
        brand = it.get("brand")
        desc = it.get("description") or ""
        line_total = it.get("price")
        unit_est   = it.get("unit_price_est")    # may be None

        # Build left block (Description)
        desc_lines = [name]
        ...
        # Wrap description and dimensions separately
        wrapped_desc = []
        for t in desc_lines:
            wrapped_desc.extend(wrap_to_width(t, (col_dim - 10) - col_desc))  # width up to dim column

        wrapped_dim  = wrap_to_width(dims_txt, (col_price - 20) - col_dim) if dims_txt else []

        # Row height = max number of lines printed in either column (at least 1)
        row_lines = max(1, max(len(wrapped_desc), len(wrapped_dim)))

        # First line: draw columns and price
        ...
        # Advance Y after the tallest column
        y += row_lines*line + 6
...
def extract_dimensions_from_note(note: str) -> dict:
    ...
    Keys you'll get when present: 'futon', 'sofa', 'desk', 'nightstand', 'chair', 'table'
    Values are strings like '98W x ?D x 55H (cm)' or '?W x ?D x 74H (cm)'.
    """
    s = " ".join(note.lower().split())  # normalize whitespace
    dims = {}

    # futon: "...futon measures around 980 mm wide and 550 mm high"
    m = re.search(r'futon[^.]*?'+DIM_MM+r'[^.]*?(?:wide|width)[^.]*?'+DIM_MM+r'[^.]*?(?:high|height)', s)
    if m:
        w_cm = mm_to_cm(m.group(1))
        h_cm = mm_to_cm(m.group(2))
        if w_cm and h_cm:
            dims['futon'] = f"{w_cm}W x ?D x {h_cm}H (cm)"

    # sectional sofa piece: "...each sofa piece is about 860 mm long and 440 mm high"
    m = re.search(r'(?:sectional\s+)?sofa[^.]*?(?:piece)?[^.]*?'+DIM_MM+r'[^.]*?(?:long|length)[^.]*?'+DIM_MM+r'[^.]*?(?:high|height)', s)
    if m:
        l_cm = mm_to_cm(m.group(1))
        h_cm = mm_to_cm(m.group(2))
        if l_cm and h_cm:
            # use length ≈ width; depth unknown
            dims['sofa'] = f"{l_cm}W x ?D x {h_cm}H (cm)"
    ...
    if m:
        h_cm = mm_to_cm(m.group(1))
        if h_cm:
            dims['nightstand'] = f"?W x ?D x {h_cm}H (cm)"

    # (optional) table pattern: round diam + height range if your notes include those
    m = re.search(r'(?:round\s+)?table[^.]*?'+DIM_MM+r'[^.]*?(?:diameter|ø)[^.]*?'+DIM_MM+r'[^.]*?(?:high|height)', s)
    if m:
        d_cm = mm_to_cm(m.group(1))
        h_cm = mm_to_cm(m.group(2))
        if d_cm and h_cm:
            dims['table'] = f"{d_cm}Ø; {h_cm}H (cm)"

    return dims

def inject_dimensions_into_items(items: list, note_dims: dict) -> list:
    """
    For any item lacking dimensions, attach a best matching dimension string parsed from the note.
    Matching is done by simple keyword heuristics on the item name.
    """
    if not items or not note_dims:
        return items

    def pick_key(name: str) -> str:
        n = (name or "").lower()
        if "futon" in n:
            return "futon"
        if "sofa" in n or "sectional" in n or "couch" in n:
            return "sofa"
        if "desk" in n or "table" in n and "desk" in note_dims:
            return "desk"
        if "table" in n:
            return "table"
        if "nightstand" in n:
...
if __name__ == "__main__":
    user_query = input("Paste the furniture note / request: ")
    customer_name, customer_no = parse_customer_from_note(user_query)
    ...
    # multi-item if long/numerous furniture words
    ...
    if not multi_note:
        ...
        if match_type != "none" and matched_docs:
            if match_type == "similar":
                print("🤝 No exact match found. Closest items:")
                for i,d in enumerate(matched_docs,1): print(f"\n[{i}] {d.metadata.get('source_file','?')}\n{d.page_content[:300]}...")
            ...
            if (not items) or all(it.get("price") in (None,"null") for it in items):
                multi_note = True
    if multi_note:
        print("ℹ️ Using custom multi-item Excel-priced BOM...")
    ...
    if not items: print("❌ Could not produce any line items."); raise SystemExit(0)
