...
- FAISS + BM25 + phrase booster (+ optional rerank)
- LLM JSON extraction + robust rule-based fallback
- Multi-line title stitching
- NEW: Brand & Dimensions extraction from a bidirectional window (above/below price)
- Dedupe near-identical items
- PDF with right-aligned price
- Output folder: generated_offers/

Run: python working.py
"""
...
# ----------------------------- #
# Rule-based fallback with title stitching + robust attributes
# ----------------------------- #
...
# NEW: robust attribute extraction from a window
...
def extract_dimensions_from_text(s: str) -> Optional[str]:
    ...
    if m:
        # return the whole matched phrase; clean double spaces
...
def extract_items_rule_based(context: str, intent: dict, max_items=3):
    ...
    for i, line in enumerate(lines):
        ...
        # stitch multi-line title around best_k
        title = lines[best_k]
        k_up = best_k - 1
        while k_up >= 0 and looks_like_title(lines[k_up]) and should_merge(lines[k_up], title):
            title = (lines[k_up] + " " + title).strip()
            k_up -= 1
        k_dn = best_k + 1
        while k_dn < len(lines) and looks_like_title(lines[k_dn]) and should_merge(title, lines[k_dn]):
            title = (title + " " + lines[k_dn]).strip()
            k_dn += 1
        ...
        # 2) fallback: infer brand from the stitched title if it contains "by <Brand>"
        if not brand:
            brand = extract_brand_from_text(title)

        # 3) fallback: if dimensions embedded in title
        if not dimensions:
            dimensions = extract_dimensions_from_text(title)
